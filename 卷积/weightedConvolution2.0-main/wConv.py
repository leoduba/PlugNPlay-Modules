...
class wConv1d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, den, stride=1, padding=1, groups=1, dilation=1, bias=False):
        ...
        # Fixed den (not trainable)
        alfa = torch.cat([
            torch.tensor(den, dtype=torch.float32),
            torch.tensor([1.0]),
            torch.flip(torch.tensor(den, dtype=torch.float32), dims=[0])
        ])
        self.register_buffer('alfa', alfa)
        self.register_buffer('Phi', alfa)

        if self.Phi.shape != self.kernel_size:
            raise ValueError(f"Phi shape {self.Phi.shape} must match kernel size {self.kernel_size}")

    def forward(self, x):
        weight_Phi = self.weight * self.Phi
...
class wConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, den, stride=1, padding=1, groups=1, dilation=1, bias=False):
        ...
        self.register_buffer('alfa', alfa)
        self.register_buffer('Phi', torch.outer(alfa, alfa))
...
class wConv3d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, den, stride=1, padding=1, groups=1, dilation=1, bias=False):
        ...
        self.register_buffer('alfa', alfa)
        self.register_buffer('Phi', torch.einsum('i,j,k->ijk', alfa, alfa, alfa))
